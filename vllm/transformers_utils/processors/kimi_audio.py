...
# TODO(wzy): Add batch inference support
class KimiAudioProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        if text_tokenizer is None:
            ...
            text_tokenizer = AutoTokenizer.from_pretrained("moonshotai/Kimi-Audio-7B-Instruct", trust_remote_code=True)
    ...
    def get_num_audio_tokens(self,
        ...
        if isinstance(audio_path, str):
            ...
            audio_length = len(audio_path['data'])

        # if model_config is None:
        #     raise ValueError("model_config for WhisperVQEncoder is None")

        pooling_kernel_size = 4 # model_config.pooling_kernel_size or 1
        conv1_stride = 1 # model_config.conv1.stride[0]
        conv2_stride = 2 # model_config.conv2.stride[0]
    ...
    def get_audio_waves(self, audio: Union[str, AUDIO_TYPE]) -> torch.Tensor:
        if isinstance(audio, str):
            ...
            raise ValueError(f"Invalid type: {type(audio)}")
    ...
    def merge_ids(self, text_input_ids, audio_input_ids):
        assert len(text_input_ids) == len(audio_input_ids)
        merged = []
        for t, a in zip(text_input_ids, audio_input_ids):
            assert (t == self.extra_tokens.kimia_text_blank or a == self.extra_tokens.kimia_text_blank)
            assert not (t == self.extra_tokens.kimia_text_blank and a == self.extra_tokens.kimia_text_blank)
            if t == self.extra_tokens.kimia_text_blank:
                merged.append(a)
            else:
                merged.append(t)
        return merged

    def handle_prompt(self, messages: KimiAContent, in_detail: bool = False) -> dict:
        ...
        text_input_ids = text_input_ids.cpu().tolist()[0]
        audio_input_ids = audio_input_ids.cpu().tolist()[0]
        input_ids = self.merge_ids(text_input_ids, audio_input_ids)
    ...
    def get_prompt(
        ...
        return self.handle_prompt(ret_msg, in_detail)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        whisper_input_feature = prompts["multi_modal_data"]["whisper_input_feature"]
